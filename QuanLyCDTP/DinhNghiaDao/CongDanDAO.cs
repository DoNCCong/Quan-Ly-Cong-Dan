    ...
    class CongDanDAO
    {
        ...
        public bool Them (CongDan cd)
        {
            ...
            string congdan = $"Insert Into CongDan(cccd,hoten,gioitinh,ngaysinh,quequan,thuongtru,tamtru,dantoc,honnhan,cccdCha,cccdMe,TienLuong,QuocTich) " +
                             $"Values('{cd.CCCD}', N'{cd.HoTen}', N'{cd.GioiTinh}', '{cd.NgaySinh.ToString().Substring(0,8)}', N'{cd.QueQuan}', N'{cd.ThuongTru}', N'{cd.TamTru}', N'{cd.DanToc}', N'{cd.HonNhan}', '{cd.CccdCha}', '{cd.CccdMe}', '{cd.Tienluong}', N'{cd.Quoctich}')";
        ...
        public bool Sua(CongDan cd)
        {
            string congdan = string.Format("Update CongDan set hoten=N'{0}',gioitinh=N'{1}',ngaysinh=N'{2}'" +
                ...
                " where cccd='{10}'"
                ,cd.HoTen,cd.GioiTinh,cd.NgaySinh,cd.QueQuan,cd.ThuongTru,cd.TamTru,cd.DanToc,cd.HonNhan,cd.CccdCha,cd.CccdMe,cd.CCCD,cd.Tienluong,cd.Quoctich);
            dbc.ThucThi(congdan);
            if (dbc.ThucThi(congdan) == null)
            {
                return false;
